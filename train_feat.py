...
def extract_rich_features(path):
    ...
    feat_list = [
        ...
        stats_over_time(mfcc_delta),
        stats_over_time(mfcc_delta2),
...
def build_feature_matrix(df, folder):
    ...
    for _, row in tqdm(df.iterrows(), total=len(df)):
        file_id = row["id"]
        label   = row["label"]
        path = os.path.join(folder, f"train_{file_id}.wav")
...
# 3) Build features
X_train_rich, y_train = build_feature_matrix(train_df, "train")
X_val_rich,   y_val   = build_feature_matrix(val_df, "train")
...
np.save("y_train_rich.npy", y_train)
np.save("X_val_rich.npy", X_val_rich)
np.save("y_val_rich.npy", y_val)

print("Saved rich feature matrices.")
